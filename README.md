...
当前包含 [`@supercarlosluo/dsh-tool-example`](packages/tool-example)，它通过 Cordis 的 `tools` 服务注册确定性的 `example_greet` Host Tool，不访问网络、文件系统或外部凭据。GitHub 仓库身份保持为 `lzx-Bill/dsh-plugins`；公开发布前仍必须按发布 Gate 重新完成当前 npm 包名对应的构建、打包、宿主运行和 registry clean install 验收。
...
`release:check` 只检查最终 npm scope 与发布环境，不执行发布；它要求 `NPM_SCOPE=@supercarlosluo`，并拒绝长期 `NPM_TOKEN`/`NODE_AUTH_TOKEN` 环境变量。npm/GitHub 写操作由维护者在 Gate 通过后手动执行。
